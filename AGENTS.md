...
- Runtime iCloud unavailability is not always terminal. If `Bros` starts while no iCloud account or connection is available, keep rechecking on the short unresolved cadence and force the active Bros tab to refresh when the runtime cloud error clears.
...
- Launch/bootstrap failures must not hard-crash when a local-only emergency path can keep the app usable. Keep volatile decoded caches conservatively bounded and purge them on background or memory warnings so iOS has less reason to terminate a backgrounded Active Workout.
...
- Active-workout relaunch continuity must preserve the user's last presentation intent. Background/user-edit/start handoffs that present Active Workout should persist a presented snapshot; explicit user minimize should persist a collapsed snapshot; restore must honor that presentation mode instead of deriving UI state from active-session existence alone.
