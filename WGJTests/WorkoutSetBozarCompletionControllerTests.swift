...
struct WorkoutSetBozarCompletionControllerTests {
    ...
    @Test
    func completesWithMergedValuesWhenHistoryHasResolved() {
        let draft = WorkoutSessionSetDraft(
            actualReps: nil,
            actualWeight: 50,
            actualLoadUnit: .lb
        ...
        #expect(resolvedDrafts[0].actualWeight == 50)
        #expect(resolvedDrafts[0].actualLoadUnit == .lb)
    }

    @Test
    func explicitPreviousFillStillOverwritesExistingMetrics() {
        let draft = WorkoutSessionSetDraft(
            actualReps: 10,
            actualWeight: 50,
            actualLoadUnit: .lb
        ...
        let resolvedDrafts = WorkoutSetBozarCompletionController.applyPreviousPerformance(
            to: [draft],
            at: 0,
            previousResolution: .resolved([0: previous])
        )
