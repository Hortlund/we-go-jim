...
- Durable Rule: In active workout, prefer a stable full exercise stack and move expensive hydration/relationship work out of render and input paths. Weight/reps keystrokes should stay in row-local draft buffers and only commit to the workout draft after debounce, focus loss, backgrounding, finish, or discard. Do not introduce lazy-row scroll-state machinery unless profiling shows row count itself is the bottleneck.
- How to Verify Next Time: Run active workout UI smokes for typed set values and minimize/restore scroll position, then review active row rendering for synchronous SwiftData relationship scans, fallback draft creation during `body`, or per-keystroke parent draft/persistence mutation.
- Status: active
