...
- Root Cause: The prior implementation treated Profile like Bros and intentionally delayed the real Profile tab tree behind `ProfileFirstFrameShellView`; the next pass only preloaded Profile after a fresh warm snapshot existed, leaving the first tap able to pay view construction while startup warmup was still active.
- Durable Rule: Profile must not use a first-frame shell on initial tab selection. Startup Profile warmup should begin during boot, Profile should pre-mount through `TabView` while warmup is active or a fresh snapshot exists, main entry should use the warmed snapshot when available, and first Profile activation must wait on the active warmup completion before deciding whether to apply the warm snapshot or load locally. Bros can keep the transition-safe shell/deferred mount because cloud feed loading is open-ended.
- How to Verify Next Time: Run `WGJTests/AppLaunchWarmupTests`, `WGJTests/WeeklyGoalWidgetTests` if widget state is also touched, and `WGJUITests/WGJUITests/testColdLaunchProfileAndBrosTabsRespondAfterSplash`; confirm Profile has zero initial mount delay, warm or actively warming Profile can preload through `TabView`, `AppWarmupState.waitForActiveProfileWarmup()` resumes on warmup finish, and `profile-first-shell` is absent while `profile-content-root` appears on the first Profile tap.
- Status: active
