...
- Durable Rule: Warm Profile/Bros snapshots may feed first content once the tab is selected, but they must not mount heavyweight deferred tab views offscreen. Startup Bros warmup must stay local/status-only and avoid remote feed fetches; real Bros CloudKit refresh belongs behind active tab activation. If a fresh warm snapshot is already ready, the selected tab may mount content immediately instead of paying an artificial first-frame delay.
- How to Verify Next Time: Run `WGJTests/AppLaunchWarmupTests` and confirm deferred Profile/Bros tabs do not preload from warm snapshots, startup Bros warm snapshot returns `.loading` rather than fetching remotely when cloud is available, and first-frame shell policy only delays real content mount when no fresh warm snapshot is ready.
- Status: active
...
- Durable Rule: Startup Profile/Bros warmups may start early through `AppBackgroundStore`, and first-install splash may wait briefly on bounded local/status warmups so Profile/Bros are ready for the first real visit. Splash must still not wait on remote Bros feed fetches or CloudKit export scheduling, and `MainTabView` must not be covered by a startup overlay. Mark startup warmups active synchronously before main is exposed. Render Profile/Bros static first-frame shells from `MainTabView` before constructing the heavy tab views. Deferred tabs must ignore `onAppear` preload and wait for settled selection; if no fresh warm snapshot is ready, mount without animation after the tab transition window. Hydration must apply warm snapshots or wait for version changes without main-context SwiftData or CloudKit work on the tap frame.
- How to Verify Next Time: Cold launch without `UITEST_SKIP_SPLASH`, confirm the tab bar and iCloud sync banner can appear after main entry, then tap Profile and Bros immediately with slow warmup hooks. Unit-test that deferred content is not scheduled from tab `onAppear`, first-install startup may wait for bounded warmups, warm snapshots remove the artificial mount delay, and cold tabs still keep a transition-safe mount delay; UI-smoke that both tabs eventually mount content under slow warmup without hanging.
- Status: active
