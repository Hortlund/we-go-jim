...
- Durable Rule: Warm Profile/Bros snapshots should feed first content and allow the deferred Profile/Bros tab view trees to preload before the first user tap. Startup Bros warmup may perform the real feed snapshot fetch during splash, but visible Bros activation must coalesce with any in-flight refresh instead of cancelling it or starting another one. If a fresh warm snapshot is already ready, the selected tab may mount and render content immediately instead of paying artificial first-frame delays.
- How to Verify Next Time: Run `WGJTests/AppLaunchWarmupTests` and `WGJTests/BrosViewModelTests`; confirm deferred Profile/Bros tabs preload from warm snapshots, startup Bros warm snapshot policy still degrades cleanly when cloud is unavailable, Profile dashboard render delay is skipped for fresh warm snapshots, and concurrent Bros refresh calls wait for the active fetch instead of duplicating it.
- Status: active
...
- Durable Rule: Share keyboard toolbar rendering through `WGJKeyboardHideButton`/`.wgjMinimalKeyboardToolbar`, and keep UIKit input accessories styled from `WGJKeyboardHideControl.buttonConfiguration()` so the hide control looks the same across SwiftUI and representable-backed fields. Attach the modifier on the screen or sheet that owns the focused fields. Do not assume a parent overlay wrapper will propagate keyboard toolbar items into every focused descendant.
- How to Verify Next Time: Run `WGJTests/AppPerformanceRuntimeTests.keyboardHideControlUsesSharedProminentAccessoryStyle`; run an active-workout UI smoke that focuses `workout-set-0-weight-field`, waits for `keyboard-hide-button`, taps it, and confirms the keyboard hides and `active-workout-elapsed-timer` returns.
- Status: active
