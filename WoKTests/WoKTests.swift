...
struct WoKTests {
    @Test
    func wokFormattersParseLocalizedDecimals() {
        let parsedDot = WoKFormatters.parseLocalizedDecimal("123.45")
        let parsedComma = WoKFormatters.parseLocalizedDecimal("123,45")

        #expect(parsedDot != nil)
        #expect(parsedComma != nil)
        #expect(abs((parsedDot ?? 0) - 123.45) < 0.001)
        #expect(abs((parsedComma ?? 0) - 123.45) < 0.001)
        #expect(!WoKFormatters.decimalString(123.45).isEmpty)
        #expect(!WoKFormatters.oneDecimalString(123.45).isEmpty)
        #expect(!WoKFormatters.integerString(123.45).isEmpty)
    }
    ...
    func durationFormatterBuildsElapsedLabels() {
        ...
        #expect(WoKDurationFormatter.elapsedString(since: reference, now: oneMinuteFive) == "01:05")
        #expect(WoKDurationFormatter.elapsedString(since: reference, now: oneHourThree) == "01:00:03")
    }
